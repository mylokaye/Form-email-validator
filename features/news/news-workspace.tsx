    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <select id="news-source" value={selectedSource} onChange={(event) => { setSelectedSource(event.target.value); void load(event.target.value); }} className="h-8 min-w-44 rounded-[10px] border border-input bg-background px-3 text-[13px] leading-4 font-medium outline-none focus-visible:ring-3 focus-visible:ring-ring/50">
      ...
      {!loading && data.items.length > 0 && (
        <Card>
          ...
          <CardContent className="divide-y px-4">
            {data.items.map((item) => (
              <article key={item.id} className="store-row py-5 first:pt-4 last:pb-4">
