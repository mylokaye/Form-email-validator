...
function SourceRow({ source, onChange, onDelete, onRefresh }: { source: Source; onChange: (source: Source) => void; onDelete: (id: number) => void; onRefresh: (id: number) => void }) {
  ...
  return <article className="flex flex-col gap-4 border-b py-4 last:border-b-0 md:flex-row md:items-start md:justify-between">
...
export function NewsManager() {
  ...
  if (access === 'signed-out') return <Card><CardContent className="space-y-3 py-12 text-center"><p className="font-medium">Sign in to manage shared sources.</p><p className="text-sm text-muted-foreground">Only the Pattens owner can change this public newsroom.</p><Link href="/signin-with-chatgpt?return_to=/news/manage/" className="inline-flex h-[50px] items-center rounded-lg bg-primary px-5 text-sm font-medium text-primary-foreground">Sign in with ChatGPT</Link></CardContent></Card>;
