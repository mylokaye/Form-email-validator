...
export function SimulationWorkspace() {
  ...
  return <div className="grid gap-6 xl:grid-cols-[minmax(340px,0.8fr)_minmax(0,1.2fr)] xl:items-start">
    <Card>
      <CardHeader className="border-b"><CardTitle>Simulation setup</CardTitle></CardHeader>
      <CardContent className="grid gap-5 pt-5">
        <div className="grid gap-2"><span className="text-sm font-medium">Proposal</span><Textarea value={proposal} onChange={(event) => { setProposal(event.target.value); setError(''); }} placeholder="Describe the proposal you want to test with an audience." className="min-h-36 resize-y" maxLength={3000} /></div>
        ...
        <div className="border-t pt-5"><div className="mb-3 flex items-center gap-2 text-sm font-medium"><Users className="h-4 w-4" />Audience panel</div><div className="grid gap-2">{selectedPersonas.map((persona) => <div key={persona.name} className="flex items-center justify-between rounded-lg border border-border bg-secondary/30 px-3 py-3"><div><p className="text-sm font-medium">{persona.name}</p><p className="text-xs text-muted-foreground">{persona.role}</p></div><span className={`rounded-full px-2.5 py-1 text-xs font-medium ${tone[persona.stance]}`}>{persona.stance}</span></div>)}</div></div>
      </CardContent>
    </Card>
    <Card className="min-h-[560px]">
      <CardHeader className="flex flex-row items-center justify-between border-b"><CardTitle>Audience debate</CardTitle><span className="rounded-full bg-secondary px-3 py-1 text-xs text-muted-foreground">{events.length} / {rounds} rounds</span></CardHeader>
      <CardContent className="grid gap-5 pt-5"><div className="grid grid-cols-3 gap-3"><Metric label="Progress" value={`${rounds ? Math.round((events.length / rounds) * 100) : 0}%`} /><Metric label="Personas" value={String(selectedPersonas.length)} /><Metric label="Response" value={result ? `${result.sentiment}%` : '—'} /></div>
